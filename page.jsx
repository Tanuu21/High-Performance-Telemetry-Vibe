...
export default function TelemetryDashboard() {
  ...
  return (
    <div className="min-h-screen bg-neo-bg p-4 md:p-8 flex justify-center items-start">
      <div className="w-full max-w-md bg-neo-bg rounded-3xl p-6 shadow-neo-out space-y-6">
        
        {/* Header / Inline Branded SVG Logo Asset */}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <Info size={14} />
            <span>About Tracker</span>
          </button>
        </div>

        {activeTab === 'dashboard' ? (
          <>
            {/* Repository Target Dropdown */}
            ...
            <div className="p-5 rounded-2xl shadow-neo-in bg-neo-bg text-center space-y-1">
              ...
              <span className="text-[11px] font-bold text-emerald-600 px-2 py-0.5 rounded-full bg-emerald-50 inline-block">Optimal Functional Rate</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl shadow-neo-out bg-neo-bg space-y-1">
                ...
                <span className="text-[10px] text-slate-400 block">Last 30d lifecycle</span>
              </div>
              <div className="p-4 rounded-2xl shadow-neo-out bg-neo-bg space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Active Unmerged PRs</span>
                <div className="text-lg font-bold text-amber-600 flex items-center space-x-1">
                  <GitPullRequest size={16} />
                  <span>{currentData.activeUnmergedPRs}</span>
                </div>
                <span className="text-[10px] text-amber-600 font-medium block">Review Required</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl shadow-neo-out bg-neo-bg flex items-center justify-between">
              ...
              <span className="text-xs font-extrabold text-blue-600 bg-blue-50 px-3 py-1 rounded-xl shadow-neo-in">{currentData.prVelocity} Integration</span>
            </div>

            {/* Live Issues List */}
            <div className="space-y-3">
              <div className="flex justify-between items-center px-1">
                <h3 className="text-sm font-black text-slate-700 uppercase tracking-wider">Live Dynamic Issue Registry</h3>
                <span className="text-xs text-slate-400 font-bold">{currentData.issues.length} Pulled</span>
              </div>
                  ...
                  >
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="space-y-1 flex-1">
                        ...
                        <p className="text-xs font-bold text-slate-700 leading-snug">{issue.title}</p>
                        <p className="text-[10px] text-slate-400">Opened by <span className="font-bold text-slate-500">@{issue.user}</span></p>
                        
                        {selectedIssue === issue.id && (
                          <div className="pt-3 mt-2 border-t border-slate-200 text-[11px] text-slate-500 font-medium space-y-1 bg-white/40 p-2 rounded-xl animate-fade-in">
                            <p>🔴 <span className="font-bold">Status:</span> Critical Block / Triaged</p>
                            <p>⚙️ <span className="font-bold">Module:</span> Core Architecture Thread</p>
                            <p>💬 <span className="font-bold">Context:</span> Element card entry click revealed full context module details debug sequence.</p>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          /* Info Area */
          <div className="p-4 rounded-2xl shadow-neo-in bg-neo-bg space-y-4 text-xs font-medium text-slate-600 leading-relaxed">
            <h3 className="text-sm font-black text-slate-700 uppercase tracking-wider">Engine Purpose</h3>
            <p>GitHealth monitors development ecosystem telemetry directly on client UI terminals using shadow-matrix layout sheets.</p>
            <div className="p-3 bg-white/50 rounded-xl space-y-1 shadow-neo-out">
              <p className="font-bold text-slate-700">Architecture Matrix:</p>
              <p>• Engine: Next.js 14 Production Node</p>
              <p>• Compiled Framework: Tailwind Core Utility</p>
              <p>• Rendering Base: Neumorphic System Kit</p>
            </div>
          </div>
        )}
      </div>
